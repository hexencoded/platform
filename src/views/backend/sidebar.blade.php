    ...
    <div class="sidebar-collapse">
        <ul class="nav" id="side-menu">
            <li @if(Route::is('backend.home')) class="active"@endif>
                <a class="active" href="{{Backend::url()}}"><i class="fa fa-dashboard fa-fw"></i> {{{ Lang::get('thor::backend.dashboard') }}}</a>
            </li>
            ...
            <li @if(Backend::requestIs('modules')) class="active"@endif>
                <a href="{{Backend::url('modules')}}"><i class="fa fa-puzzle-piece fa-fw"></i> {{{ Lang::get('thor::backend.modules') }}}</a>
            </li>
            @endif
            @if(Entrust::can('list_users'))
            <li @if(Backend::requestIs('users')) class="active"@endif>
                <a href="{{Backend::url('users')}}"><i class="fa fa-user fa-fw"></i> {{{ Lang::get('thor::backend.users') }}}</a>
            </li>
            @endif
            @if(Entrust::can('list_roles'))
            <li @if(Backend::requestIs('roles')) class="active"@endif>
                <a href="{{Backend::url('roles')}}"><i class="fa fa-group fa-fw"></i> {{{ Lang::get('thor::backend.roles') }}}</a>
            </li>
            @endif
            @if(Entrust::can('list_permissions'))
            <li @if(Backend::requestIs('permissions')) class="active"@endif>
                <a href="{{Backend::url('permissions')}}"><i class="fa fa-lock fa-fw"></i> {{{ Lang::get('thor::backend.permissions') }}}</a>
            </li>
            @endif
            @if(Entrust::can('list_languages'))
            <li @if(Backend::requestIs('languages')) class="active"@endif>
                <a href="{{Backend::url('languages')}}"><i class="fa fa-flag fa-fw"></i> {{{ Lang::get('thor::backend.languages') }}}</a>
            </li>
            @endif
